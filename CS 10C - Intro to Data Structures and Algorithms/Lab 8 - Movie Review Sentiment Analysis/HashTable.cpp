...
int HashTable::computeHash(const string &s) {
	...
	int num = 0;
    for (int i = 0; i < s.size(); ++i) //cycle through the string
    {
        num = num + (num * 4) * s[i]; //multiplicative string hash using hashMultiplier of four
    }

    num = num % size; //take the mod of the index
...
{
	int index = computeHash(s); //get the index of where it will need to go

	//first check to see if the string is in the hashtable
    for(list<WordEntry>::iterator i = hashTable[index].begin(); i !=hashTable[index].end(); ++i)
    {
        if(s == i->getWord()) //if the string is already in the hash
        {
            i->addNewAppearance(score); //addNewAppearence with the score to the WordEntry
            return; //break out of the function
        }
    }
    //if there is no duplicates, then create a new Entry and push it on the list at the appropriate array index
    WordEntry new_word = WordEntry(s,score); //create a new entry
    hashTable[index].push_back(new_word); //push back the new word at the index
}
...
double HashTable::getAverage(const string &s) {
    ...
    int index = computeHash(s); //get the index of where its supposed to go
    for(list<WordEntry>::iterator i = hashTable[index].begin(); i !=hashTable[index].end(); ++i) //cycle through the hash table
    {
        if(s == i->getWord()) //if word is found
        {
            return i->getAverage();
        }
    }
    //if its not there, return 2.0
    return 2.0;
}
