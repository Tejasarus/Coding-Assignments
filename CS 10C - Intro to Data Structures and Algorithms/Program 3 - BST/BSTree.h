...
class BSTree {
    ...
    public:
        ...
        private:
        void pre(Node*) const;
        void pOrder(Node*) const;
        void iOrder(Node*) const;
        Node* isDupe(string, Node*);
        Node* remove(string, Node*);
        Node* maxNode(Node* n)//same thing as largest, but returns the node instead
        {
           while(n->right != 0)
           {
              n = n->right;
        ...
        Node* minNode(Node* n)//same thing as smallest, but returns the node instead
        {
           while(n->left != 0)
           {
              n = n->left;
